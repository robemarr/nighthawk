...
Router.prototype.onSubmit = function (e, el) {
	...
	formData.forEach(function (value, key) {
		var data = formData.getAll(key);
		body[key] = data.length <= 1 ? value : data;
	});
	// Run the request processing
 ...
 * @param {Boolean} replace - Should this replace or push?
 */
Router.prototype._processPostRequest = function (_url, body) {
	// Normalize the url object
	var url = this._normalizeUrl(_url);
	var update = this._updateLocation(url);
	// If the update is valid process it
	if (update) {
